...
### Phase 1: バトルの通しプレイ可能な状態 ✅完了
- [x] 1部隊あたり最大30ユニットからなる味方部隊の実装
- [x] 部隊データ管理方法の実装
- [x] フィールド上で高さルールを順守した状態でのバトル通しプレイ

### Phase 2: ユニット種別実装 🔶部分完了
- [x] 各兵種ごとに異なる攻撃範囲・移動速度などのバリエーション実装
- [x] 兵種ごとのユニットサイズ別スプライトパターンの仕様策定
- [ ] ユニットアニメーションパターンの生成実装
- **詳細な進捗**: `docs/STATUS_AND_PLAN.md` 参照
...
**近接攻撃**（歩兵・騎士など）:
- 高低差3グリッド未満（48 world units未満）まで近接攻撃可能
- 3グリッド以上（≧48 world units）の高低差がある場合は近接攻撃不可、「届かない!」メッセージ表示
- 射程内にあっても高低差が大きい場合は移動して接近する必要あり

**遠隔物理攻撃**（弓・銃・砲: bowArc, longArc, siege）:
- **高さ判定**: 弓矢の弧の高さに基づく制限
- 弧の高さ算出: `baseArcHeight = (1 + 6 * distFactor) * TILE_HEIGHT`（`distFactor = 1 - dist/maxRange`）
  - 近距離: 弧が高く、高所の敵にも当たりやすい
  - 遠距離: 弧が低く、高所の敵に当たりにくい
- ターゲットが弧の頂点より高い場合: 矢は弧の頂点まで飛んで消失し、「届かない」メッセージ表示
- 高所からの攻撃: ダメージ+15%/段
- 低所からの攻撃: ダメージ-15%/段（最低50%）

**魔法攻撃**（魔術師: aoe）:
- **高さ制限なし** — 魔法は高低差を無視して射程内のターゲットに発動する
- 魔法弾は直接ターゲットに向かう

**回復魔法**（僧侶: heal）:
- **高さ制限なし** — 回復魔法は高低差を無視して射程内の味方に発動する
- 聖なる光で表現

**ドラゴンブレス**（ドラゴン: breath）:
- **高さ制限なし** — ブレスは高低差を無視して発動する

**高さ計算共通**:
- ユニットの高さは `Math.max(地形高さ, 建物高さ)` で計算される
- 高さ計算には `mapSystem.getHeight()` を使用

**実装**:
- `combat.js` の `meleeCombat()` で近接攻撃の高低差チェック
- `combat.js` の `rangedCombat()` で遠隔攻撃の弧ベース高低差チェック
- `MAX_MELEE_HEIGHT_DIFF = 3 * TILE_HEIGHT`（近接攻撃の高低差制限）
- 物理弾は `isPhysicalProjectile` フラグで判定（bowArc, longArc, siege）
- 魔法系（aoe, heal）は高さチェックをスキップ
...
| 2026-03-04 | 高さベース攻撃制限を実装: 近接は≧3グリッド不可、弓/銃/砲は弧の高さ基準、魔法/回復は高さ無視 |
| 2026-03-04 | rangedCombatのデッドコード（重複定義）を削除 |
| 2026-03-04 | 配置画面の戦闘開始ボタン修正（z-index、autoPlaceUnitsのロバスト性向上） |
...
| 2026-03-04 | 7.2 戦闘ルールを更新: ユニットタイプ別の高さベース攻撃制限を追記（魔法/回復の高さ制限免除、弓の弧ベース判定） |
