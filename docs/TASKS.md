...
## Phase 2.5: コード品質改善 (2026-05-12)

Codex レビュー対応による10項目の改善。

### TICKET-Q01: isInAttackRange誤呼び出し修正 ✅完了
- combat.js: isMeleeReachable に MELEE_RANGE_TYPES ガード・方向変換・サイズ対応・高低差チェック統合
- テスト: test/combat-integration.test.mjs (20テスト追加)

### TICKET-Q02: FORMATION_INFO import修正 ✅完了
- combat.js: formation.js から正しく FORMATION_INFO をインポート

### TICKET-Q03: innerHTML安全化 ✅完了
- ui-manager.js, scene-manager.js: 動的データに esc() エスケープ適用

### TICKET-Q04: postMessage origin検証 ✅完了
- building-designer.html: targetOrigin '*' → window.location.origin
- map-editor.html: 受信側で event.origin 検証追加

### TICKET-Q05: RenderingEngine3D stop/dispose ✅完了
- rendering3d.js: stop(), dispose() 追加。イベントリスナーのリーク解消

### TICKET-Q06: UnitSpawner兵種渡しタイミング修正 ✅完了
- unit-spawner.js, scene-manager.js: warlordData の class/type を正規化

### TICKET-Q07: MAP_W/MAP_H固定依存のAPI統一 ✅完了
- map-dimensions.js: getMapDimensions(), isWithinMap() 追加
- pathfinding.js, formation.js: mapSystem.isValidCoord 利用

### TICKET-Q08: DataURL廃止 ✅完了
- sample_mountain.js: 2つのJPEG DataURL (274KB) → assets/maps/ に抽出 (52KB)

### TICKET-Q09: CI追加 ✅完了
- .github/workflows/ci.yml: Node 22 + npm test
- package.json: ci スクリプト追加

### TICKET-Q10: rendering3d.js分割 + ゴミ削除 ✅完了
- rendering-coords.js: 座標変換純粋関数を抽出
- ゴミファイル10件削除 (backup, fix scripts, temp files)
